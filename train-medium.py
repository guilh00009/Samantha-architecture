...
if not args.resume:
    # New run
    run = wandb.init(project='gpt3-small-fineweb', name='fineweb-training-lion-meduim')
    # Store the run id for future resuming
    run_id = wandb.run.id
    os.makedirs('./emergency_checkpoint_meduim', exist_ok=True)
    with open('./emergency_checkpoint_meduim/run_id.txt', 'w') as f:
        f.write(run_id)
else:
    # Resume run
    if os.path.exists('./emergency_checkpoint_meduim/run_id.txt'):
        with open('./emergency_checkpoint_meduim/run_id.txt', 'r') as f:
            run_id = f.read().strip()
        # Initialize wandb run with the stored run_id
        run = wandb.init(project='gpt3-small-fineweb',
                         id=run_id,
                         resume="allow")
        print(f"Resuming WandB run with id {run_id}")
    else:
        # If no run_id is found, start a new run or handle accordingly
        run = wandb.init(project='gpt3-small-fineweb', name='fineweb-training-lion-meduim')
...
# Load datasets ### TODO: optimize this code
### idea: array of dump names
...
#dumps = ["CC-MAIN-2024-10"]
#for _ in dumps:
#    ...

dataset = interleave_datasets([dataset1, dataset2, dataset3, dataset4, dataset5, dataset6]).shuffle()
...
train_dataloader = DataLoader(train_dataset, batch_size=12, num_workers=0) # MANAGE BATCH SIZE HERE #12

# Load the validation dataset (Wikitext-2). Loss may be higher but this will show how the world model performs on unseen data
...
validation_dataloader = DataLoader(lm_validation_dataset, batch_size=12, num_workers=0) # MANAGE BATCH SIZE HERE #12

# slightly modifies architecture from modelling_gpt3dev.py
...
# init a new model because we're training from scratch, may be overriden with --resume
model = GPT3DevLMHeadModel(config)

# Custom Weight Initialization because GPT-3 has biases
...
model=torch.compile(model, backend="aot_eager", mode="max-autotune")
model.to(device)
...
# Replaced AdamW with Lion for better speed
optimizer = Lion(
    model.parameters(),
    lr=6e-4,
    betas=(0.9, 0.99),
...
gradient_accumulation_steps = 16 #16 #4
max_grad_norm = 1.0

# we define global step because we have a --reume flag that will override it, now it's 0 because we train from scratch
global_step = 0

# Resume training if --resume flag is used
if args.resume:
    checkpoint_dir = './emergency_checkpoint_meduim'
...
try:
    while True:
        ...
        if global_step > 0 and global_step % save_steps == 0:
            save_path = f'./gpt3-medium-fineweb-step-{global_step}'
    ...
    save_path = f'./gpt3-medium-fineweb-lion-step-{global_step}'
    model.save_pretrained(save_path)
    tokenizer.save_pretrained(save_path)
...
final_save_path = './gpt3-medium-fineweb_v9_lion'
