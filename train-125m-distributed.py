...
def train(rank, world_size, args):
    ...
    if rank == 0:
        if not args.resume:
            run = wandb.init(project='gpt3-small-fineweb', name='fineweb-training-600,000')
            run_id = wandb.run.id
            os.makedirs('./emergency_checkpoint', exist_ok=True)
            with open('./emergency_checkpoint/run_id.txt', 'w') as f:
                f.write(run_id)
        else:
            if os.path.exists('./emergency_checkpoint/run_id.txt'):
                with open('./emergency_checkpoint/run_id.txt', 'r') as f:
                    run_id = f.read().strip()
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=6, num_workers=0)
    ...
    validation_dataloader = DataLoader(lm_validation_dataset, batch_size=6, num_workers=0, sampler=val_sampler)
    ...
    gradient_accumulation_steps = 4 #4 works well
    ...
    if rank == 0:
        final_save_path = './gpt3-small-fineweb_v9'
...
# Spawn processes
def main():
    parser = argparse.ArgumentParser(description='Training script with resume functionality and DDP.')
